...
const { optionsConfig } = require('@hap-toolkit/shared-utils/compilation-config')
...
// 能使用抽取公共js功能的调试器最低版本
const SPLIT_CHUNKS_SUPPORT_VERSION_FROM = 1070
 ...
 * @param {boolean} [options.splitChunksMode=undefined] - 抽取公共JS
...
module.exports = function genWebpackConf(options, mode) {
  ...
  if (hapConfigPath) {
    try {
      customConf = require(hapConfigPath)
      if (typeof customConf.cli === 'object') {
        cli = customConf.cli
        options = Object.assign({}, cli, options)
      }
    } catch (err) {
  ...
  // 设置抽取公共js
  if (options.splitChunksMode === optionsConfig.splitChunksModeEnum.SMART) {
    colorconsole.warn(
